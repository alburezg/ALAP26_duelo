...
Usaremos cuatro insumos, todos en `data/colombia/`:

```{r m3-setup}
dcol <- "data/colombia"

# (a) Estructura de parentesco: parientes vivos por año, sexo/edad de Focal
#     y sexo/edad del pariente. Es la SALIDA del modelo DemoKin (Módulo 2)
#     aplicado a Colombia; no se versiona por su tamaño. Si no existe, la
#     generamos con prep/03 (corre kin2sex; tarda ~10 min) y se reutiliza.
archivo_kin <- file.path(dcol, "col_kin_1985_2018.parquet")
if (!file.exists(archivo_kin)) source("prep/03_build_kinship_colombia.R")
...
# (c) Datos demográficos colombianos (población y factor ax).
demo <- read_parquet(file.path(dcol, "col_demo_1950_2018.parquet"))
pop <- demo %>% select(year, sex, age, pop, ax)

# (d) Proporción de inmigrantes (se excluyen del riesgo de duelo).
...
n_homicidios <- read_csv(file.path(dcol, "col_homicidios_totales.csv"),
  show_col_types = FALSE
)$n_muertes[1]
```

La **estructura de parentesco** (`kin`) es la salida del modelo bisexual variable
en el tiempo del Módulo 2, corrido para Colombia con salida por período
1985–2018 y para Focal de ambos sexos. Por su tamaño (~20 MB) no se versiona: se
genera con `prep/03_build_kinship_colombia.R` (o automáticamente la primera vez
que se ejecuta este módulo). El código que la produce es equivalente a:

> **Nota sobre el período.** Para que el modelo corra rápido en el taller, aquí
> lo iniciamos en **1950** (usando solo datos del WPP). El artículo original lo
> inicia en 1900, incorporando mortalidad histórica anterior a 1950. Como DemoKin
> supone una población estable antes del primer año, iniciar en 1950 da
> estimaciones de parentesco lejano algo **mayores** que las publicadas; las
> cifras son, por tanto, cercanas pero no idénticas a las del artículo.

```{r m3-kin-gen, eval=FALSE}
# (equivalente a lo que hicimos en el Módulo 2, pero para 1985-2018 y ambos sexos)
kin2sex(
...
```{r m3-labels}
etiquetas_kin <- c(
  m = "Madres y padres", d = "Hijas e hijos", s = "Hermanas y hermanos",
...
```{r m3-cum-table, echo=FALSE}
knitr::kable(
  prevalencia %>% transmute(
    `Pariente / grupo` = etiqueta,
    `Personas en duelo (millones)` = round(millones, 2),
    `% de la población` = round(pct, 1)
  ),
  caption = "Prevalencia acumulada del duelo por homicidio en 2018 (personas que han perdido al menos un pariente del tipo/grupo indicado)."
)
```
