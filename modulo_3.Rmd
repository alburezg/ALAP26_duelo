...
del pariente. No es un dato externo, sino la **salida del modelo del Módulo 2**
(de dos sexos y variable en el tiempo) aplicado a las tasas colombianas
`demo`, con salida por período 1985–2018 y para Focal de ambos sexos.

Como el cálculo tarda unos minutos, lo corremos **una sola vez**, guardamos el
resultado en un archivo y lo reutilizamos: si el archivo ya existe, simplemente lo
leemos (esa es nuestra "caché"). El código es el mismo `kin2sex()` del Módulo 2,
generalizado a los ocho tipos de pariente y a los dos sexos de Focal:

> **Nota sobre el período.** Para que el modelo corra rápido en el taller, aquí lo
> iniciamos en **1950** (usando solo datos del WPP). El artículo original lo inicia
> en 1900, incorporando mortalidad histórica anterior a 1950. Como DemoKin supone
> una población estable antes del primer año (véase "condiciones de frontera" en el
> Módulo 2), iniciar en 1950 da estimaciones de parentesco lejano algo **mayores**
> que las publicadas; las cifras son, por tanto, cercanas pero no idénticas a las
> del artículo.
...
if (!file.exists(archivo_kin)) {
  # Pasar una tasa del formato tidy (una fila por edad-año) a matriz (edad x año)
  a_matriz <- function(variable, sexo) {
    demo %>%
      filter(sex == sexo) %>%
      select(age, year, valor = all_of(variable)) %>%
      arrange(age, year) %>%
  ...
  # kin2sex corre para un sexo de Focal a la vez; lo llamamos para cada sexo
  correr_kin <- function(sexo_focal) {
    kin2sex(
      pf = a_matriz("px", "f"), pm = a_matriz("px", "m"),
      ff = a_matriz("fx", "f"), fm = a_matriz("fx", "m"),
      nf = a_matriz("nx", "f"), nm = a_matriz("nx", "m"),
      time_invariant = FALSE,
      output_period = 1985:2018,
      output_kin = c("c", "d", "gd", "gm", "m", "n", "a", "s"),
      sex_focal = sexo_focal,
      birth_female = 0.5
    )$kin_full %>%
      as_tibble() %>%
      mutate(sex_focal = sexo_focal)
  }

  kin <- bind_rows(correr_kin("f"), correr_kin("m")) %>%
    transmute(
      year = as.integer(year), sex_focal,
      age_focal = as.integer(age_focal), kin, sex_kin,
      age_kin = as.integer(age_kin), living
    ) %>%
    filter(living >= 1e-6) # descartar celdas prácticamente vacías
...
![Operaciones booleanas sobre los dos estados de duelo. Cada región lleva su probabilidad bajo el supuesto de independencia. La intersección ("y") es la categoría "cercana y extendida"; toda el área sombreada es la unión ("o"). Adaptado del material metodológico de @acosta_weaponizing_2026.](venn_duelo.png)
