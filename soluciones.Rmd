### Soluciones · ejercicio del Módulo 4 (resuelto para Costa Rica) {#soluciones}
...
> **Marcador de posición.** Costa Rica no tuvo un conflicto como el colombiano y
> aún no disponemos de tasas de homicidio por edad. Por eso aquí usamos, como
> sustituto, la **mortalidad por todas las causas** del WPP
> (`data/homicide_mortality_rates.csv`). Los resultados reflejan entonces el
> duelo por *toda* la mortalidad (no solo la violenta) y sirven para ilustrar el
> mecanismo; cuando dispongamos de las tasas reales de homicidio, se reemplaza ese
> archivo y el código no cambia.
...
anio_final <- 2020 # último año del análisis
...
# Tasas de mortalidad por homicidio (marcador de posición = todas las causas).
...
hombre). Tarda ~10 min; lo calculamos una vez, lo guardamos y lo reutilizamos.
...
if (!file.exists(archivo_kin)) {
  ...
  kin_f <- kin2sex(
    ...
    time_invariant = FALSE, output_period = 1990:anio_final,
    output_kin = c("c", "d", "gd", "gm", "m", "n", "a", "s"), sex_focal = "f"
  )
  kin_m <- kin2sex(
    pf = pf, pm = pm, ff = ff, fm = fm, nf = nf, nm = nm,
    time_invariant = FALSE, output_period = 1990:anio_final,
    output_kin = c("c", "d", "gd", "gm", "m", "n", "a", "s"), sex_focal = "m"
  )
  kin <- bind_rows(
...
incidencia_familia %>%
  ...
  labs(
    title = paste0("Incidencia anual del duelo (", iso, ", marcador de posición)"),
...
# muertes totales del período (con las tasas placeholder = todas las causas)
muertes_total <- read_csv("data/homicide_mortality_rates.csv", show_col_types = FALSE) %>%
  filter(iso3 == iso, year %in% 1990:anio_final) %>%
